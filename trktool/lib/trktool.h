// $Header:   Y:/archives/dv/Tracker/trkar/trktool.h_v   1.95   23 Oct 2000 22:54:14   stansc  $
//
...
//
// TrkProjectLoginEx requires a valid trkHandle as above.  Each 
// parameter is required for this function.  The serverName should
// a server that was defined with the server wizard in Tracker.
//
...
TRKAPI TrkProjectLoginEx(
	TRK_HANDLE					trkHandle,				// Input
	TRK_CSTR					userName,				// Input (see above)
	TRK_CSTR					password,				// Input (see above)
	TRK_CSTR					projectName,			// Input (see above)
	TRK_CSTR					serverName);			// Input (see above)
...
//
// The same holds for the TrkInitProjectListEx function.
//
...
typedef TRK_UINT		TRK_STRING_OPTION;
